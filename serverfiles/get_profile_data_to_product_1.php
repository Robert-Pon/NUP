...
if(($i==1)|| ($login = "-" && $password ="-")){
    $sql1 = "SELECT id FROM passwords WHERE id = '$userID' AND password = '$password'";
    $resp1 = mysqli_query($conn, $sql1);
    if(($resp1->fetch_object()->id==$userID)|| ($login = "-" && $password ="-")){
        $getInfo = "SELECT name, familyname, city, school, email, number, description FROM people WHERE id = '$id2'";
