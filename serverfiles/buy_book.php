...
if($i==1 && $id1 = $userID){
    ...
    if($resp1->fetch_object()->id==$userID){
        ...
        if($responseObject == "0"){
            mysqli_query($conn, "UPDATE books SET reservation = '1', seen = 'false', res_id = '$id1'  WHERE id = '$id2' AND reservation = '0'");
            $res1 = mysqli_query($conn, "SELECT res_id FROM books WHERE id = '$book_id'");
            ...
            if($res_id ==  $userID){
                ...
                $responseObject = array("type" => "110", "id" => strval($i));
            }
        }else{
