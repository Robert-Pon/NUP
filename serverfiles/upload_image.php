...
if($i==1 && $id1 = $userID){
    ...
    if($resp1->fetch_object()->id==$userID){
        ...
        $name = strval(count($array)).".jpg";
        if(!file_exists("xIsqweriosamuiwrunsdkjfkaurpw/$id1")){
            mkdir("xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id", 0777, true);
            move_uploaded_file($_FILES['image']['tmp_name'], "xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id/$name");
        }else if(!file_exists("xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id")){
            mkdir("xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id", 0777, true);
            move_uploaded_file($_FILES['image']['tmp_name'], "xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id/$name");
        }else{
            move_uploaded_file($_FILES['image']['tmp_name'], "xIsqweriosamuiwrunsdkjfkaurpw/$id1/$book_id/$name");
        ...
        array_push($array, strval("/$id1/$book_id/".strval(count($array))).".jpg");
