...
if(($i==1 && $id1 = $userID)|| $id1 = "-"){
    ...
    if(($resp1->fetch_object()->id==$userID)|| $id1 = "-"){
        $search1 = "\"".$search."\"";
        $sql = "SELECT b.* FROM books AS b JOIN people AS p ON p.id = b.person_id WHERE b.too_search LIKE '%$search%' AND NOT b.person_id = '$userID' AND b.reservation = '0' ORDER BY -p.nups";
