...
if(($i==1 && $id1 = $userID)|| $id1 = "-"){
    ...
    if(($resp1->fetch_object()->id==$userID)|| $id1 = "-"){
        $sql = "SELECT b.*, p.*, l.* FROM books AS b JOIN people AS p ON p.id = b.person_id JOIN localizations AS l ON l.id = p.school WHERE b.too_search LIKE '%$search%' AND NOT b.person_id = '$userID' AND b.reservation = '0' AND l.x BETWEEN '".doubleval($x1)."' AND '".doubleval($x)."' AND l.y BETWEEN '".doubleval($y1)."' AND '".doubleval($y)."'";
